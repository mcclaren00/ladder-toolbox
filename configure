...
main() {
    ...
    if [[ "${verify}" == 1 ]]; then
        ...
        envsubst < "${PROJECT_DIR}/initial/.sops.yaml" \
            > "${PROJECT_DIR}/.sops.yaml"
        # generate cluster settings
        envsubst < "${PROJECT_DIR}/initial/cluster/cluster-settings.yaml" \
            > "${PROJECT_DIR}/cluster/config/cluster-settings.yaml"
        envsubst < "${PROJECT_DIR}/initial/cluster/flux-cluster.yaml" \
            > "${PROJECT_DIR}/cluster/flux/flux-system/flux-cluster.yaml"
        envsubst < "${PROJECT_DIR}/initial/cluster/kube-vip-daemonset.yaml" \
            > "${PROJECT_DIR}/cluster/core/kube-system/kube-vip/daemon-set.yaml"
        # generate cluster secrets
        envsubst < "${PROJECT_DIR}/initial/cluster/cluster-secrets.sops.yaml" \
            > "${PROJECT_DIR}/cluster/config/cluster-secrets.sops.yaml"
        envsubst < "${PROJECT_DIR}/initial/cluster/cert-manager-secret.sops.yaml" \
        ...
        envsubst < "${PROJECT_DIR}/initial/terraform/secret.sops.yaml" \
        ...
        envsubst < "${PROJECT_DIR}/initial/ansible/kube-vip.yml" \
...
setup_github_webhook() {
    ...
    if [[ "${WEBHOOK_SECRET}" != "ignored" ]]; then
        ...
        cp -rf  "${PROJECT_DIR}/initial/cluster/flux-system" "${PROJECT_DIR}/cluster/apps/"

        envsubst < "${PROJECT_DIR}/initial/cluster/flux-system/webhooks/github/secret.sops.yaml" \
