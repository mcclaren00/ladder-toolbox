...
record_ipv4=$(curl -s -X GET \
    "https://api.cloudflare.com/client/v4/zones/${zone_id}/dns_records?name=${BOOTSTRAP_CLOUDFLARE_DOMAIN}&type=A" \
    -H "X-Auth-Email: ${CLOUDFLARE_EMAIL}" \
    -H "X-Auth-Key: ${CLOUDFLARE_APIKEY}" \
    -H "Content-Type: application/json" \
)
...
update_ipv4=$(curl -s -X PUT \
    ...
    -H "X-Auth-Key: ${CLOUDFLARE_APIKEY}" \
    -H "Content-Type: application/json" \
    --data "{\"id\":\"${zone_id}\",\"type\":\"A\",\"proxied\":true,\"name\":\"${BOOTSTRAP_CLOUDFLARE_DOMAIN}\",\"content\":\"${current_ipv4}\"}" \
)
