...
import '.App.css';
const ipfsClient = require('ips-api')
const ipfs = ipfsClient({ host: 'ipse.infura.io', port: '5001', protocol: 'https' })
...
class App extends Component {
    ...
    async loadWeb3() {
        if (window.ethereum) {
            ...
            window.alert('Cannot run Ethereum on web browser. Try using metamask')
        }
    }
    ...
    async loadBlockchainData() {
        ...
        if (networkData) {
            ...
            window.alert('Smart contract was not deployed to network')
        }
    }
    ...
    onSubmitClick = async (event) => {
        ...
        if (this.state.buffer) {
            const file = await ipfsClient.add(this.state.buffer)
